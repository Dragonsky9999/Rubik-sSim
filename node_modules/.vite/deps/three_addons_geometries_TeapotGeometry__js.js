import { B as BufferAttribute, Fs as Vector3, Is as Vector4, V as BufferGeometry, Yr as Matrix4 } from "./three.core-BCF3aXvo.js";
//#region node_modules/three/examples/jsm/geometries/TeapotGeometry.js
/**
...
var TeapotGeometry = class extends BufferGeometry {
	...
	*/
	constructor(size = 50, segments = 10, bottom = true, lid = true, body = true, fitLid = true, blinn = true) {
		...
		const maxHeight2 = 3.15 * (blinn ? 1 : blinnScale) / 2;
		...
		for (let i = 0; i < 3; i++) mgm[i] = new Matrix4();
		...
		let uvCount = 0;
		let indexCount = 0;
		for (let surf = minPatches; surf < maxPatches; surf++) if (lid || surf < 20 || surf >= 28) {
			for (let i = 0; i < 3; i++) {
				for (let r = 0; r < 4; r++) for (let c = 0; c < 4; c++) {
					g[c * 4 + r] = teapotVertices[teapotPatches[surf * 16 + r * 4 + c] * 3 + i];
					if (fitLid && surf >= 20 && surf < 28 && i !== 2) g[c * 4 + r] *= 1.077;
			...
			for (let sstep = 0; sstep <= segments; sstep++) {
				...
				for (let tstep = 0; tstep <= segments; tstep++) {
					...
					norm.normalize();
					if (vert[0] === 0 && vert[1] === 0) normOut.set(0, vert[2] > maxHeight2 ? 1 : -1, 0);
					else normOut.set(norm.x, norm.z, -norm.y);
			...
			for (let sstep = 0; sstep < segments; sstep++) for (let tstep = 0; tstep < segments; tstep++) {
...
//# sourceMappingURL=three_addons_geometries_TeapotGeometry__js.js.map